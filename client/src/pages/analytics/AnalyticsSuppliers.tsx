...
const AnalyticsSuppliers = () => {
 ...
 {quality.length === 0 ? (
 <div className="p-8 text-center text-secondary">No supplier batch data available.</div>
 ) : (
 <table className="min-w-full divide-y divide-border">
 <thead className="bg-card">
 <tr>
 <th className="px-5 py-4 text-left font-bold text-secondary uppercase text-xs tracking-wider">Supplier Party</th>
 <th className="px-5 py-4 text-right font-bold text-secondary uppercase text-xs tracking-wider">Production Batches</th>
 ...
 <tbody className="divide-y divide-border bg-canvas/20">
 {quality.map((s: any) => (
 <tr key={s.supplierName} className="hover:bg-canvas transition-colors">
 <td className="px-5 py-4 whitespace-nowrap text-sm font-bold text-primary">{s.supplierName}</td>
 <td className="px-5 py-4 whitespace-nowrap text-right text-sm text-secondary font-mono">{s.totalBatches}</td>
 <td className="px-5 py-4 whitespace-nowrap text-right text-sm font-medium text-primary font-mono">{s.totalInputKg.toLocaleString()}</td>
 <td className="px-5 py-4 whitespace-nowrap text-right text-sm font-bold text-status-danger font-mono">{s.totalRejectionKg.toLocaleString()}</td>
 <td className="px-5 py-4 whitespace-nowrap text-right">
 <div className="flex items-center justify-end gap-2">
 {s.rejectionRate > 5 && <ShieldAlert className="h-4 w-4 text-status-danger" />}
 <span className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-bold ${s.rejectionRate > 5 ? 'bg-status-danger/20 text-status-danger' : 'bg-status-success/20 text-status-success'}`}>{s.rejectionRate}%</span>
 </div>
 </td>
 </tr>
 ))}
