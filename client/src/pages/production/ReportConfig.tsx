    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div>
                ...
                <p className="text-gray-500">Manage plant targets, utility costs, and automated reporting</p>
            </div>

            {msg.text && (
                <div className={`p-4 rounded-lg font-medium border ${msg.type === 'success' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                    {msg.text}
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="max-w-xs">
                        <label className="block text-sm font-medium text-gray-700 mb-2">Target Yield (%)</label>
                        <div className="relative">
                            <input
                                ...
                                className="w-full border-gray-300 rounded-lg pr-8 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm"
                            />
                            <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <span className="text-gray-500">%</span>
                            </div>
                        </div>
                        <p className="mt-2 text-xs text-gray-500">Batches falling below this target will be flagged in reports.</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <p className="text-sm text-gray-500 mb-6">These baseline rates are used to estimate the daily operating cost of dyeing batches.</p>
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    className="w-full border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 shadow-sm"
                                />
                            </div>
                        </div>
                        <div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4 border-b pb-2">
                        <Mail className="h-5 w-5 text-emerald-500" /> Automated Daily Reports
                    </h3>
                    <p className="text-sm text-gray-500 mb-4">Add email addresses that should receive the automated 6 PM PDF production report.</p>
                    ...
                    <div className="flex gap-2 mb-4 max-w-md">
                        <input
                            ...
                            className="flex-1 border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 shadow-sm"
                        />
                        <button
                            type="button"
                    ...
                    <div className="flex flex-wrap gap-2">
                        {formData.dailyReportEmails.length === 0 && <span className="text-sm text-gray-400 italic">No recipients configured.</span>}
                        {formData.dailyReportEmails.map((email) => (
                            <div key={email} className="flex items-center gap-2 bg-indigo-50 text-indigo-700 border border-indigo-100 px-3 py-1.5 rounded-full text-sm font-medium">
                                {email}
                                <button type="button" onClick={() => handleEmailRemove(email)} className="text-indigo-400 hover:text-indigo-900 font-bold">&times;</button>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex justify-end">
                    <button
                        ...
                        className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-lg font-bold shadow-md shadow-indigo-200 transition disabled:opacity-50"
                    >
