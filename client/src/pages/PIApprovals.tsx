...
const PIApprovals = () => {
 ...
 {pis.map((pi) => (
 <div key={pi._id} className="bg-surface rounded-lg shadow border border-border overflow-hidden">
 ...
 <th className="text-right text-xs font-medium text-secondary uppercase">Quantity</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border">
 {pi.items.map((item) => (
 <tr key={item._id}>
 <td className="py-2 text-sm text-primary">{item.materialId?.name}</td>
 ...
 <div className="mb-3">
 <label htmlFor={`remarks-${pi._id}`} className="block text-xs font-medium text-secondary mb-1">
 ...
 className="w-full text-sm border-border rounded-md shadow-sm focus:border-primary focus:ring-primary"
 ...
 className="flex items-center px-4 py-2 border border-red-300 text-red-700 rounded hover:bg-danger/10 text-sm font-medium"
 >
 <XCircle className="w-4 h-4 mr-2" /> Reject
 </button>
 <button
 onClick={() => handleStatusUpdate(pi._id, 'APPROVED')}
 className="flex items-center px-4 py-2 bg-green-600 text-primary rounded hover:bg-green-700 text-sm font-medium"
 >
 ...
 {pis.length === 0 && <p className="text-center text-secondary">No PI requests found.</p>}
 </div>
 )}
 </div>
 );
