...
const ReturnMaterial = () => {
 ...
 <h2 className="text-2xl font-bold font-heading text-primary flex items-center gap-2 mb-6">
 ...
 {message.text && (
 <div className={`p-4 rounded-lg mb-6 flex items-center ${message.type === 'error' ? 'bg-danger/10 text-danger' : 'bg-success/10 text-success'}`}>
 ...
 <div>
 <label className="block text-xs font-bold text-secondary uppercase tracking-wide mb-1">Select Material</label>
 <select
 ...
 <div>
 <label className="block text-xs font-bold text-secondary uppercase tracking-wide mb-1">Quantity to Return</label>
 <input
 type="number"
 min="0.01"
 step="0.01"
 ...
 <div>
 <label className="block text-xs font-bold text-secondary uppercase tracking-wide mb-1">Reason (Optional)</label>
 <textarea
 value={reason}
 onChange={(e) => setReason(e.target.value)}
 className="w-full p-3 bg-background border border-border rounded-lg focus:ring-2 focus:ring-primary focus:outline-none h-24"
