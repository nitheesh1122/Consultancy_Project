...
import { generatePDF, buildFileName, type PDFTableColumn } from '../lib/pdfGenerator';
...
const PDFDownloadButton: React.FC<PDFButtonProps> = ({ type, data, fileName }) => {
    const handleDownload = () => {
        ...
        const isMRSId = data._id ? `MRS-${data._id.slice(-6).toUpperCase()}` : undefined;
        const isPIId = data._id ? `PI-${data._id.slice(-6).toUpperCase()}` : undefined;

        const mrsSignatures = [
            { label: 'Requested By', name: data.supervisorId?.username || '' },
            { label: 'Verified By (Store)', name: '' },
            { label: 'Issued By', name: '' },
        ];

        const piSignatures = [
            { label: 'Raised By', name: data.storeManagerId?.username || '' },
            { label: 'Approved By (Admin)', name: '' },
        ...
        generatePDF({
            ...
            fileName: fileName || buildFileName(
                isMRS ? 'Material-Request-Slip' : 'Product-Inward-Order',
                isMRS ? isMRSId : isPIId
            ),
            tableColumns: columns,
            tableData,
            metaData,
            statusKey: 'status',
            signatures: isMRS ? mrsSignatures : piSignatures,
        });
    };
    ...
    return (
        <button
            ...
            className="flex items-center text-primary hover:text-primary text-xs font-semibold uppercase tracking-wide"
            title="Download PDF"
        >
